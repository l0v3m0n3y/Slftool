...
class Slftool {
private:
    std::string api_base = "https://slftool.github.io/";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        request.headers().add(U("Host"), U("slftool.github.io"));
...
public:
    ...
    pplx::task<json::value> get_slftool_data() {
        http_client client(utility::conversions::to_string_t(api_base), client_config);
        return make_api_call("/data.json","GET");
    }
};
